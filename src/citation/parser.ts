/**
 * Parse Nordic legal citation strings into structured objects.
 *
 * Supported formats:
 *   - L. nr. 90/2018
 *   - 90/2018 14. gr.
...
/** Icelandic statute pattern: [L. nr.] 90/2018 [14. gr.] */
const IS_LAW_PATTERN = /^(?:l(?:ög)?\.\s*(?:nr\.\s*)?)?(\d+[a-z]?\/\d{4})\s*(?:(\d+\s*[a-z]?)\.\s*gr\.)?\s*$/iu;
...
const CASE_IS_HRD_PATTERN = /^(HRD|HÆSTIRÉTTUR)\.?\s+(\d{4})\s*(?:[, ]\s*(?:bls\.?|nr\.?)\s*(\d+))?/iu;

/**
 * Parse a Nordic legal citation string.
 *
...
export function parseCitation(citation: string): ParsedCitation {
  ...
  // Icelandic law format (e.g., "L. nr. 90/2018 14. gr." or "90/2018")
  const isLawMatch = trimmed.match(IS_LAW_PATTERN);
  if (isLawMatch) {
    const result: ParsedCitation = {
      raw: citation,
      type: 'statute',
      document_id: isLawMatch[1],
      valid: true,
    };
    if (isLawMatch[2]) {
      result.section = isLawMatch[2].replace(/\s+/g, ' ').trim();
    }
    return result;
  }
  ...
  const hrdMatch = trimmed.match(CASE_IS_HRD_PATTERN);
  if (hrdMatch) {
    return {
      raw: citation,
      type: 'case_law',
      document_id: `HRD ${hrdMatch[2]}`,
      page: hrdMatch[3] || undefined,
      valid: true,
    };
  }
...
export function detectDocumentType(citation: string): DocumentType | null {
  ...
  if (/^(?:l(?:ög)?\.\s*(?:nr\.\s*)?)?\d+[a-z]?\/\d{4}\b/u.test(trimmed)) return 'statute';
  if (trimmed.startsWith('prop.')) return 'bill';
  if (trimmed.startsWith('sou ')) return 'sou';
  if (trimmed.startsWith('ds ')) return 'ds';
  if (/^(nja|hfd|ad|md|mig|hrd|hæstiréttur)\s/i.test(trimmed)) return 'case_law';
