#!/usr/bin/env node
/**
 * Icelandic Legal Citation MCP Server
 *
 * Provides tools for querying Icelandic statutes, case law,
...
import {
  ...
  GetIcelandicImplementationsInput,
} from './tools/get-swedish-implementations.js';
...
const DB_ENV_VAR = 'ICELANDIC_LAW_DB_PATH';
...
const TOOLS: Tool[] = [
  {
    ...
    description: `Search Icelandic statutes and regulations by keyword.
...
Returns matched provisions with snippets, relevance scores, and document metadata.`,
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Search query in Icelandic or English. Supports FTS5 syntax.',
        },
        document_id: {
          type: 'string',
          description: 'Filter to a specific statute by Law number (e.g., "2018:218")',
        },
        status: {
          type: 'string',
  ...
  {
    name: 'get_provision',
    description: `Retrieve a specific provision from a Icelandic statute.

Specify the Law number and either chapter+section or provision_ref directly.
Examples:
...
Omit chapter/section/provision_ref to get all provisions in the statute.`,
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description: 'Law number (e.g., "2018:218")',
        },
        chapter: {
          type: 'string',
  ...
  },
  {
    ...
    description: `Search Icelandic court decisions (rattsfall).
...
Searches case summaries and keywords. Filter by court (HD, HFD, AD, etc.) and date range.`,
  ...
  },
  {
    ...
    description: `Get preparatory works (forarbeten) for a Icelandic statute.
...
Essential for understanding legislative intent behind statutory provisions.`,
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description: 'Law number of the statute (e.g., "2018:218")',
        },
      },
      required: ['document_id'],
    },
  },
  {
    ...
    description: `Validate a Icelandic legal citation against the database.
...
Formats:
  ...
  {
    name: 'check_currency',
    description: `Check if an Icelandic statute or provision is in force (current or historical).
...
Returns the document's status (in_force, amended, repealed, not_yet_in_force), dates, and warnings. Provide as_of_date for historical evaluation.`,
    inputSchema: {
      ...
      properties: {
        document_id: {
          type: 'string',
          description: 'Law number (e.g., "90/2018")',
        },
        provision_ref: {
          type: 'string',
...
Essential for understanding which EU law an Icelandic statute is based on.`,
    inputSchema: {
      ...
      properties: {
        ...
        sfs_number: {
          type: 'string',
          description: 'Legacy alias for law_number',
        },
        include_articles: {
          type: 'boolean',
...
Essential for finding Icelandic law corresponding to EU requirements.`,
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description: 'Deprecated alias for get_icelandic_implementations.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
          type: 'string',
          description: 'EU document ID',
        },
        primary_only: {
          type: 'boolean',
          description: 'Return only primary implementing statutes (default: false)',
...
Use this for exploratory searches like "data protection" or "privacy" to find relevant EU law.`,
    inputSchema: {
      ...
      properties: {
        ...
        has_icelandic_implementation: {
          type: 'boolean',
          description: 'Filter by Icelandic implementation existence',
        },
        has_swedish_implementation: {
          type: 'boolean',
          description: 'Legacy alias for has_icelandic_implementation',
        },
        limit: {
          type: 'number',
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  ...
  return {
    resources: [
      {
        uri: 'case-law-stats://icelandic-law-mcp/metadata',
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (uri === 'case-law-stats://icelandic-law-mcp/metadata') {
    try {
      const db = getDb();
          ...
          contents: [
            {
              ...
              text: JSON.stringify(
                {
                  status: 'no_data',
                  message: 'No Icelandic case law data has been synced yet in this build.',
                  last_sync_date: null,
                  ...
                  source: {
                    ...
                    license: 'N/A',
                    attribution: 'Case law sync source is not configured for this dataset.',
                  },
      ...
      const stats = {
        ...
        source: {
          name: syncMeta?.source || 'Not configured',
          url: 'https://www.althingi.is/',
          license: 'N/A',
          attribution: 'Case law sync source is not configured for this dataset.',
        },
