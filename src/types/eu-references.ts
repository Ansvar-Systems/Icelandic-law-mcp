 ...
 * EU References Types
 *
 * Types for EU law cross-references in Icelandic legislation.
 */
...
export interface EUReference {
  ...
  source_id: string;
  document_id: string;           // Icelandic law identifier (e.g., "90/2018")
  provision_id?: number;
...
export interface IcelandicImplementation {
  law_number: string;
  law_title: string;
  short_name?: string;
  status: string;
...
/**
 * Backward-compatible alias retained to avoid breaking existing callers.
 */
export type SwedishImplementation = IcelandicImplementation;
