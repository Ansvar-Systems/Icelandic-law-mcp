...
import { getIcelandicImplementations, GetIcelandicImplementationsInput } from './get-swedish-implementations.js';
...
Returns matched provisions with snippets, relevance scores, and document metadata.`,
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search query in Icelandic or English. Supports FTS5 syntax.' },
        document_id: { type: 'string', description: 'Filter to a specific law by number (e.g., "90/2018")' },
  ...
  {
    name: 'get_provision',
    description: `Retrieve a specific provision from an Icelandic statute.
...
Specify the law number and either chapter+section or provision_ref directly.`,
  ...
  },
  {
    ...
    description: `Search Icelandic court decisions. Filter by court and date range.`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: `Get preparatory works for an Icelandic statute.`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: `Validate an Icelandic legal citation against the database. Parses the citation, checks existence, and returns warnings.`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: `Format an Icelandic legal citation per standard conventions (full, short, or pinpoint).`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        law_number: { type: 'string', description: 'Law number (e.g., "90/2018")' },
        sfs_number: { type: 'string', description: 'Legacy alias for law_number' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: { type: 'string' },
        primary_only: { type: 'boolean' },
        in_force_only: { type: 'boolean' },
      },
      required: ['eu_document_id'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_icelandic_implementation: { type: 'boolean', description: 'Filter by Icelandic implementation existence' },
        has_swedish_implementation: { type: 'boolean', description: 'Legacy alias for has_icelandic_implementation' },
