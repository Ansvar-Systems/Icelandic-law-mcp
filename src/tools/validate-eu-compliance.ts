/**
 * validate_eu_compliance — Check Icelandic statute's EU compliance status.
 */
...
/**
 * Validate EU compliance status for an Icelandic statute or provision.
 *
...
export async function validateEUCompliance(
  ...
  const lawNumber = input.law_number ?? input.sfs_number;
  const lawIdPattern = /^(\d{4}:\d+|\d+[a-z]?\/\d{4})$/i;

  // Validate law number format
  if (!lawNumber || !lawIdPattern.test(lawNumber)) {
    throw new Error(
      `Invalid SFS number format / Icelandic law number format: "${lawNumber ?? ''}". ` +
      `Expected "NN/YYYY" (e.g., "90/2018") or legacy "YYYY:NNN" (e.g., "2018:218").`
    );
  }
  ...
  `).get(lawNumber) as { id: string; title: string; status: string } | undefined;

  if (!statute) {
    throw new Error(`Statute ${lawNumber} not found in database`);
  }
  ...
  if (input.provision_ref) {
    ...
    `).get(lawNumber, input.provision_ref) as { id: number } | undefined;

    if (!provision) {
      throw new Error(
        `Provision ${lawNumber} ${input.provision_ref} not found in database`
      );
    }
  ...
  const params: (string | number)[] = [lawNumber];
  ...
  if (rows.length === 0) {
    ...
    recommendations.push(
      'Statute references repealed EU directives. Review whether Icelandic law has been updated to implement current EU requirements.'
    );
