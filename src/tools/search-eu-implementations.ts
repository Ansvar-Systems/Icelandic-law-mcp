...
export interface SearchEUImplementationsInput {
  ...
  has_icelandic_implementation?: boolean;
...
export interface SearchEUImplementationsResult {
  results: Array<{
    ...
    icelandic_statute_count: number;
    /** Legacy alias retained for backward compatibility. */
...
/**
 * Search for EU directives/regulations with Icelandic implementation information.
 *
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.community,
      ed.celex_number,
      COUNT(DISTINCT er.document_id) AS icelandic_statute_count,
  ...
  // Filter by Icelandic implementation existence (or legacy Swedish alias)
  const hasImplementationFilter =
    input.has_icelandic_implementation ?? input.has_swedish_implementation;

  if (hasImplementationFilter !== undefined) {
    if (hasImplementationFilter) {
      sql += ` HAVING icelandic_statute_count > 0`;
    } else {
      sql += ` HAVING icelandic_statute_count = 0`;
    }
  }
  ...
  const results = rows.map(row => ({
    ...
    icelandic_statute_count: row.icelandic_statute_count,
    swedish_statute_count: row.icelandic_statute_count,
