#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Icelandic Law MCP server.
 *
 ...
 *   npm run build:db:paid                   # Step 2: Add paid tables + metadata
 *   (optional) ingest paid datasets into the added tables
 *
...
function finalizeDatabaseArtifact(db: Database.Database): void {
  // Keep WAL for migration speed, then finalize to DELETE mode for
...
function buildPaidTier(): void {
  console.log('Building paid-tier extensions for Icelandic Law MCP...\n');
  ...
  console.log(`    2. preparatory_works_full -- needs Icelandic preparatory-works source integration (future)`);
